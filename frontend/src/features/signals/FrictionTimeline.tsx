...
import { getFrictionEvents } from '../../api/signals';
import type { FrictionEvent } from '../../api/signals';
...
function dotColor(event: FrictionEvent): string {
...
export function FrictionTimeline({ softwareId, stageTopic }: { softwareId: string; stageTopic?: string }) {
  const [expandedIdx, setExpandedIdx] = useState<number | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['friction-events', softwareId, stageTopic],
    queryFn: () => getFrictionEvents(softwareId, stageTopic),
    enabled: !!softwareId,
  });
  ...
  if (!events.length) {
    return (
      <p className="text-xs text-gray-400 py-3">No friction events found for this stage.</p>
    );
  }
