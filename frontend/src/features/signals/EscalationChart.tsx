...
import { getEscalationRate } from '../../api/signals';
import { TrendingDown, TrendingUp, Minus, Info, AlertTriangle } from 'lucide-react';
...
function EscalationTooltip({ active, payload, label }: any) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-md text-xs">
      ...
      <div className="space-y-1">
        <div className="flex justify-between gap-4">
          ...
          <span className="font-medium text-amber-600">{point.rate}%</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-gray-500">Escalated / Total</span>
          <span className="text-gray-700">{point.escalation_count} / {point.total_threads}</span>
        </div>
      ...
      {point.top_escalations.length > 0 && (
        <div className="mt-2 pt-2 border-t border-gray-100">
          <p className="text-gray-500 mb-1">Escalated topics:</p>
          {point.top_escalations.map((topic, i) => (
...
export function EscalationChart({ softwareId, stageTopic }: { softwareId: string; stageTopic?: string }) {
  const { data: escalation, isLoading } = useQuery({
    queryKey: ['escalation-rate', softwareId, stageTopic],
    queryFn: () => getEscalationRate(softwareId, stageTopic),
    enabled: !!softwareId,
  });
  ...
  if (!escalation || !escalation.points.length) return null;

  const hasAnyData = escalation.points.some((p) => p.total_threads > 0);
  if (!hasAnyData) return null;

  const peer = escalation.peer;
  const commentary = escalation.commentary;
  ...
  const merged: MergedPoint[] = escalation.points.map((p) => ({
    ...p,
  ...
  const allValues = merged.flatMap((p) => [p.rate, p.peerRate ?? 0]);
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm text-gray-500">Severity Escalation Rate</p>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <AreaChart data={merged} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
          <defs>
            ...
            {peer && (
              <linearGradient id="escalationPeerGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      <div className="flex items-center gap-4 mt-2 px-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-500">Your escalation rate</span>
        </div>
        {peer && (
          <div className="flex items-center gap-1.5">
      ...
      <div className="flex items-start gap-2 mt-2 px-1">
        ...
        <p className="text-[10px] text-gray-400 leading-relaxed">
          Escalation rate measures how often issues increase in severity after initial assessment.
          Frequent escalation signals that problems are compounding or initial severity assessments
          are too optimistic. A declining rate means triage is getting more accurate.
        </p>
      </div>
    </div>
  );
